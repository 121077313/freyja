	...
	protected boolean doDecode(IoSession session, IoBuffer in,
		...
		if (!session.containsAttribute(WebSocketUtils.SessionAttribute)) {
			...
			if (tryWebSockeHandShake(session, in, out)) {
				...
				&& true == (Boolean) session
						.getAttribute(WebSocketUtils.SessionAttribute)) {
...
//		 System.out.println(s);
	 ...
	 * 4.2.2 RFC 6455).
	 */
	private boolean tryWebSockeHandShake(IoSession session, IoBuffer in,
		...
		try {
			...
			String challengeAccept = WebSocketUtils
					.getWebSocketKeyChallengeResponse(socketKey);
			WebSocketHandShakeResponse wsResponse = WebSocketUtils
					.buildWSHandshakeResponse(challengeAccept);
