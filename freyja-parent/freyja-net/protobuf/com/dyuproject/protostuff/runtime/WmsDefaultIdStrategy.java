 ...
 * serialized representation may be not very efficient.
 * 
 * @author Leo Romanoff
 * @author David Yu
 * 
 */
public final class WmsDefaultIdStrategy extends IdStrategy {
	...
	final ConcurrentHashMap<String, Delegate<?>> delegateMapping = new ConcurrentHashMap<String, Delegate<?>>();

	/**
	 * Registers a pojo. Returns true if registration is successful or if the
	 * same exact schema was previously registered. Used by
	 * {@link WmsRuntimeSchema#register(Class, Schema)}.
	 */
	...
	/**
	 * Used by {@link WmsRuntimeSchema#map(Class, Class)}.
	 */
	...
	protected <T> Delegate<T> transferDelegateId(Input input, Output output,
		...
		final Delegate<T> delegate = (Delegate<T>) delegateMapping
				.get(className);
		if (delegate == null)
	...
	protected <T> Schema<T> writePojoIdTo(Output output, int fieldNumber,
				...
				synchronized (this) {
					if ((schema = this.schema) == null) {
						if (Message.class.isAssignableFrom(typeClass)) {
							...
							// create new
							this.schema = schema = WmsRuntimeSchema.createFrom(
									typeClass, strategy);
						}
					}
				}
			...
			if (pipeSchema == null) {
				synchronized (this) {
					if ((pipeSchema = this.pipeSchema) == null) {
						this.pipeSchema = pipeSchema = WmsRuntimeSchema
